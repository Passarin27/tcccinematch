...
async function mostrarListas() {
  ...
  for (const lista of listas) {
    ...
    for (const filme of filmes) {
      ...
      const generoNome = detalhes.genres?.[0]?.name || "—";
      ...
      card.innerHTML = `
        ...
        <span class="coracao ativo" onclick="removerFavorito(this, ${lista.id}, ${filme.id})">❤</span>
      `;
...
async function removerFavorito(el, listaId, filmeId) {
  const res = await fetch(`${API}/listas/${listaId}/filmes/${filmeId}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`
    }
